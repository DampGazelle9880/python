...
for _ in range(num_of_enemies):
    enemy_img.append(pygame.image.load('enemy.jpg'))
...
def game_over_text():
    over_text = over_font.render("GAME_OVER: " + str(score_value),True,(255,255,255))
...
def is_collision(enemy_x,enemy_y,bullet_x,bullet_y):
    ...
    return distance < COLLION_DISTANCE
running = True
while running:
    screen.fill((0,0,0))
    ...
    for i in range(num_of_enemies):
        ...
        if is_collision(enemy_x[i],enemy_y[i],bullet_x,bullet_y):
            ...
            enemy_y[random.randint(ENEMY_START_Y_MIN,ENEMY_START_Y_MAX)]
